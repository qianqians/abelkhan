...
hub_call_hub_reg_hub_direct_cb::hub_call_hub_reg_hub_direct_cb(uint64_t _cb_uuid, std::shared_ptr<hub_call_hub_rsp_cb> _module_rsp_cb) {
    cb_uuid = _cb_uuid;
    module_rsp_cb = _module_rsp_cb;
}

std::shared_ptr<hub_call_hub_reg_hub_direct_cb> hub_call_hub_reg_hub_direct_cb::callBack(std::function<void()> cb, std::function<void()> err) {
    sig_reg_hub_direct_cb.connect(cb);
    sig_reg_hub_direct_err.connect(err);
    return shared_from_this();
}

void hub_call_hub_reg_hub_direct_cb::timeout(uint64_t tick, std::function<void()> timeout_cb) {
    auto _module_rsp_cb = module_rsp_cb;
    auto _cb_uuid = cb_uuid;
    TinyTimer::add_timer(tick, [_module_rsp_cb, _cb_uuid](){
