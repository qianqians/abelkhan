...
public:
	...
	void heartbeat_client(int64_t ticktime) {
		...
		for (auto _client : heartbeat_client){
			...
			auto _caller = std::make_shared<caller::gate_call_logic>(client_map[client_uuid].second);
	...
	void refresh_client(std::shared_ptr<juggle::Ichannel> _client, int64_t ticktime) {
		client_time[_client] = ticktime;
	}

	void reg_client(std::string uuid, std::shared_ptr<juggle::Ichannel> _client, std::shared_ptr<juggle::Ichannel> _logic, int64_t ticktime) {
...
private:
	...
	std::map<std::string, std::pair<std::shared_ptr<juggle::Ichannel>, std::shared_ptr<juggle::Ichannel> > > client_map;
