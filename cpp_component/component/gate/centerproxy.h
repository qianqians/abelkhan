...
public:
	...
	void reg_server(struct name_info& _name_info, std::function<void()> conn_callback) {
		...
		_center_caller->reg_server_mq("gate", "gate", _name_info.name)->callBack([this, &_name_info, conn_callback]() {
			is_reg_sucess = true;
			conn_callback();
	...
	void reconn_reg_server(struct name_info& _name_info, std::function<void()> conn_callback) {
		_center_caller->reconn_reg_server_mq("gate", "gate", _name_info.name)->callBack([this, &_name_info, conn_callback]() {
			is_reg_sucess = true;
			conn_callback();
			spdlog::trace("connect center server sucess!");
		}, []() {
